...
def check_if_in_reference(accession):
    """
    Return True when an accession already looks like a reviewed UniProtKB
    accession and can be kept without further conversion.

    This used to depend on the KSTAR reference list, but that created an
    unnecessary import dependency for the web app. For dataset prep, the
    important behavior is to avoid remapping already-reviewed UniProt IDs.
    """
    return is_swissprot_accession(accession.split('-')[0])
